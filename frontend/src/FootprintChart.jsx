...
/* ─── computeLTP: Liquidity Trap Pressure (professional orderflow) ───
 * DP = delta / (total_vol + ε), EMA(3) smoothed
 * AS = |delta| / (|priceChange| + ε), log(1+AS)/log(1+AS_max), absorption sign
 * DD = -corr(price_change, delta_change) over 5 bars → +1 divergence, -1 confirmation
 * OWP = PutWrite - CallWrite (indices), tanh(OWP/OWP_std)
 * LTP = 0.28*DP + 0.32*AS + 0.25*DD + 0.15*OWP
 */
function computeLTP(candles, hftSeries, symbol) {
  ...
  for (let i = 0; i < candles.length; i++) {
    ...
    raw.push({ open_time: c.open_time, dp, asRaw, asSign, ddNorm, owpRaw });
  }
  ...
  for (let i = 0; i < raw.length; i++) {
    const r = raw[i];
    const asNorm = asLogMax > 0
      ? (Math.log(1 + r.asRaw) / asLogMax) * r.asSign
      : 0;
...
/* ─── computeMII: Momentum Ignition Index (professional orderflow) ───
 * DA = EMA(delta,2) - EMA(delta,5), tanh(DA/delta_std)
 * VE = (Vol - AvgVol20) / StdVol20, tanh(VE/2)
 * PE = |Close-Open| / (High-Low), PE_norm = 2*PE - 1
 * MII = 0.45*DA + 0.30*VE + 0.25*PE
 */
function computeMII(candles) {
  ...
  for (let i = 0; i < candles.length; i++) {
    ...
    raw.push({ open_time: c.open_time, da, ve, pe, delta, priceChange: close - open });
  }
